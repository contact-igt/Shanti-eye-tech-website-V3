  ...
  try {
    ...
    if (!resp.ok || response?.success === false || response?.ok === false) {
      return {
        ok: false,
        status: resp.status,
        message: typeof response?.message === "string" ? response.message : "Google Sheets submission failed",
        response,
      };
    }
    ...
    return {
      ok: false,
      message: error instanceof Error ? error.message : "Failed to connect to Google Sheets service",
    };
  }
}
